...
module "ec2" {
  ...
  environment    = var.environment
  vpc_id         = local.deploy_vpc ? module.vpc[0].vpc_id : var.vpc_id
  subnet_ids     = local.deploy_vpc ? module.vpc[0].public_subnet_ids : var.subnet_ids
...
module "s3" {
  ...
  environment = var.environment
  bucket_name = "hello-testui-17-may"
}
